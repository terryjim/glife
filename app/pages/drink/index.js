...
import {menuList,hisRouters} from '../../config/index'
...
class Drink extends React.Component {
  ...
  itemOnChanged = ({viewableItems, changed}) => {
    ...
    if (firstItem && firstItem.item) {
        ...
        let index = this.props.drinkList.tarlist.findIndex(item=>{return item.name===name})
...
//     })
  ...
  clearing=()=>{
    let data =[]
    const {drinkOrder,user,drinkList} = this.props
    const {prodlist} = drinkList
    for(let i in drinkOrder){
      let index = prodlist&&prodlist.findIndex(item=>{return item.targetId===i})
      if(index>=0){
        drinkOrder[i] && drinkOrder[i].map((v,inx)=>{
  ...
  render() {
    ...
    const {drinkList,specprodList,drinkOrder} = this.props
    const {prodlist,reclist,tarlist} = drinkList
    if(drinkList&&prodlist&&prodlist[prodlist.length-1]){
    ...
    return (
      <View style={defaultStyles.container}>
        ...
        <View style={{height:44,marginTop:27,marginLeft:18}}>
            <Text style={{fontSize:19,fontWeight:'bold',color:'#4A4A4A'}}>精品推荐</Text>
        </View>
        <View>
...
const mapStateToProps = (state) => {
  let user = state.user
  let drinkList = {}
  if(state.drinkList&&state.drinkList.serviceid==41){
    drinkList = state.drinkList
  }
  let drinkOrder = state.drinkOrder
  let specprodList = state.specprodList

  return {drinkList,drinkOrder,specprodList,user}
}
Drink = connect(mapStateToProps)(Drink)
